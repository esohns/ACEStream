...
#include "test_i_pop_receive_defines.h"
#include "test_i_pop_receive_eventhandler.h"
#if defined (GUI_SUPPORT)
#if defined (GTK_USE)
#include "test_i_pop_receive_gtk_callbacks.h"
#elif defined (WXWIDGETS_USE)
#include "test_u_pop_receive_ui.h"
#endif
#endif // GUI_SUPPORT
#include "test_i_pop_receive_common_modules.h"
#include "test_i_pop_receive_signalhandler.h"
#include "test_i_pop_receive_stream.h"
#include "test_i_pop_receive_stream_common.h"

const char stream_name_string_[] = ACE_TEXT_ALWAYS_CHAR ("POPReceive_Stream");
#if defined (GUI_SUPPORT)
                     ...
                     )
{
  ...
  while ((option = argumentParser ()) != EOF)
  {
    switch (option)
    {
      ...
      case 'v':
      {
        mode_out = STREAM_POPRECEIVE_PROGRAMMODE_PRINT_VERSION;
        break;
      }
#if defined (_DEBUG)
      case 'z':
...
do_work (
         ...
         struct Stream_POPReceive_UI_CBData& CBData_in,
#if defined (WXWIDGETS_USE)
         ...
         struct Stream_POPReceive_Configuration& configuration_in,
         const ACE_Sig_Set& signalSet_in,
         const ACE_Sig_Set& ignoredSignalSet_in,
         Common_SignalActions_t& previousSignalActions_inout,
         Stream_POPReceive_SignalHandler& signalHandler_in)
{
  ...
  Stream_AllocatorHeap_T<ACE_MT_SYNCH,
                         struct POP_AllocatorConfiguration> heap_allocator;
...
//  } // end IF
  ...
  Test_I_POPReceive_Connection_Manager_t* connection_manager_p =
    TEST_I_POPReceive_CONNECTIONMANAGER_SINGLETON::instance ();
           ...
           ACE_TCHAR* argv_in[])
{
  ...
  {
    case STREAM_POPRECEIVE_PROGRAMMODE_PRINT_VERSION:
    {
    ...
    case STREAM_POPRECEIVE_PROGRAMMODE_NORMAL:
      break;
    default:
    {
      ACE_DEBUG ((LM_ERROR,
...
#if defined (GUI_SUPPORT)
//  struct Stream_POPReceive_UI_CBData* ui_cb_data_p = NULL;
  struct Stream_POPReceive_UI_CBData ui_cb_data;
...
//  GError* error_p = NULL;
  ...
  css_profile_path += ACE_DIRECTORY_SEPARATOR_STR;
  css_profile_path +=ACE_TEXT_ALWAYS_CHAR (TEST_U_Stream_pop_receive_UI_CSS_FILE);
...
//                                             GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
  ...
  ACE_NEW_NORETURN (iapplication_p,
                    Stream_pop_receive_V4L_WxWidgetsApplication_t (toplevel_widget_name_string_,
                                                               argc_in,
                                                               Common_UI_WxWidgets_Tools::convertArgV (argc_in,
                                                                                                       argv_in),
                                                               COMMON_UI_WXWIDGETS_APP_CMDLINE_DEFAULT_PARSE));
  Stream_pop_receive_V4L_WxWidgetsApplication_t::IINITIALIZE_T* iinitialize_p =
    dynamic_cast<Stream_pop_receive_V4L_WxWidgetsApplication_t::IINITIALIZE_T*> (iapplication_p);
  // *NOTE*: this sets ui_cb_data.UIState
  iinitialize_p->initialize (ui_cb_data);
  Stream_pop_receive_V4L_WxWidgetsIApplication_t* iapplication_2 =
    dynamic_cast<Stream_pop_receive_V4L_WxWidgetsIApplication_t*> (iapplication_p);
  ACE_ASSERT (iapplication_2);
  const Stream_pop_receive_V4L_WxWidgetsApplication_t::CONFIGURATION_T& configuration_r =
    iapplication_2->getR_2 ();
  ACE_ASSERT (configuration_r.UIState);
  ui_state_p =
    const_cast<Stream_pop_receive_V4L_WxWidgetsApplication_t::CONFIGURATION_T&> (configuration_r).UIState;
...
#endif // GTK_USE
#endif // GUI_SUPPORT
  Stream_POPReceive_SignalHandler signal_handler;
