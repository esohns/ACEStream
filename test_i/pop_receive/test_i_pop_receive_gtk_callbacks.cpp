...
stream_processing_function (void* arg_in)
{
  ...
  struct Stream_POPReceive_UI_ThreadData* thread_data_p =
      static_cast<struct Stream_POPReceive_UI_ThreadData*> (arg_in);
  ...
  struct Stream_POPReceive_UI_CBData* cb_data_p =
...
//    ACE_ASSERT (progress_bar_p);
  ...
  {
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("failed to Stream_POPReceive_Stream::initialize(), aborting\n")));
    goto error;
  } // end IF
...
idle_initialize_UI_cb (gpointer userData_in)
{
  ...
  //Stream_POPReceive_StreamConfiguration_t::ITERATOR_T iterator_2 =
...
idle_update_progress_cb (gpointer userData_in)
{
  ...
  // sanity check(s)
  struct Stream_POPReceive_ProgressData* progress_data_p =
      static_cast<struct Stream_POPReceive_ProgressData*> (userData_in);
...
#endif /* __cplusplus */
void
button_connect_clicked_cb (GtkButton* button_in,
                           gpointer userData_in)
{
  STREAM_TRACE (ACE_TEXT ("::button_connect_clicked_cb"));
  ...
  ACE_OS::memset (&ui_cb_data_p->progressData.statistic, 0, sizeof (POP_Statistic_t));
  GtkProgressBar* progress_bar_p =
    GTK_PROGRESS_BAR (gtk_builder_get_object ((*iterator).second.second,
                                              ACE_TEXT_ALWAYS_CHAR (TEST_I_UI_GTK_PROGRESSBAR_NAME)));
  ACE_ASSERT (progress_bar_p);
