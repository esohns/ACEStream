...
//                                   struct Stream_AllocatorConfiguration,
  ...
  friend class Stream_MessageAllocatorHeapBase_T<ACE_MT_SYNCH,
                                                 ...
                                                 Stream_CameraML_Message_T<DataType,
                                                                           SessionDataType>,
                                                 Stream_CameraML_SessionMessage_T<Stream_CameraML_Message_T<DataType,
                                                                                                            SessionDataType>,
                                                                                  SessionDataType> >;
 ...
 protected:
  // convenient types
  typedef Stream_CameraML_Message_T<DataType,
                                   SessionDataType> OWN_TYPE_T;

  // copy ctor to be used by duplicate() and child classes
  // --> uses an (incremented refcount of) the same datablock ("shallow copy")
  Stream_CameraML_Message_T (const OWN_TYPE_T&);

 private:
  ACE_UNIMPLEMENTED_FUNC (Stream_CameraML_Message_T ())
  // *NOTE*: to be used by message allocators
  Stream_CameraML_Message_T (Stream_SessionId_t,
                             ACE_Data_Block*, // data block to use
                             ACE_Allocator*,  // message allocator
                             bool = true);    // increment running message counter ?
  Stream_CameraML_Message_T (Stream_SessionId_t,
                             ACE_Allocator*); // message allocator
  ACE_UNIMPLEMENTED_FUNC (Stream_CameraML_Message_T& operator= (const Stream_CameraML_Message_T&))
};
