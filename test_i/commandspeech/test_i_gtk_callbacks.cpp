...
continue_:
  ...
  //        (ACE_OS::strcmp (string_2, ACE_TEXT_ALWAYS_CHAR ("hw:CARD=MID,DEV=0")) == 0)   ||
      ...
      gtk_list_store_set (listStore_in, &iterator,
                          ...
                          1, card_i,
                          2, ACE_TEXT_ALWAYS_CHAR (device_name_string.c_str ()),
                          -1);
...
stream_processing_function (void* arg_in)
{
  ...
  switch (thread_data_base_p->mediaFramework)
  {
    case STREAM_MEDIAFRAMEWORK_DIRECTSHOW:
    {
      ...
      Common_IGetR_2_T<Test_I_CommandSpeech_DirectShow_SessionData>* iget_p =
        dynamic_cast<Common_IGetR_2_T<Test_I_CommandSpeech_DirectShow_SessionData>*> (directshow_ui_cb_data_p->stream);
      ACE_ASSERT (iget_p);
      directshow_session_data_p =
        &const_cast<Test_I_CommandSpeech_DirectShow_SessionData&> (iget_p->getR_2 ());
    ...
    case STREAM_MEDIAFRAMEWORK_MEDIAFOUNDATION:
    {
      ...
      Common_IGetR_2_T<Test_I_CommandSpeech_MediaFoundation_SessionData>* iget_p =
        dynamic_cast<Common_IGetR_2_T<Test_I_CommandSpeech_MediaFoundation_SessionData>*> (mediafoundation_ui_cb_data_p->stream);
      ACE_ASSERT (iget_p);
      mediafoundation_session_data_p =
        &const_cast<Test_I_CommandSpeech_MediaFoundation_SessionData&> (iget_p->getR_2 ());
  ...
  Common_IGetR_2_T<Test_I_CommandSpeech_ALSA_SessionData>* iget_p =
    dynamic_cast<Common_IGetR_2_T<Test_I_CommandSpeech_ALSA_SessionData>*> (ui_cb_data_p->stream);
  ACE_ASSERT (iget_p);
  session_data_p =
    &const_cast<Test_I_CommandSpeech_ALSA_SessionData&> (iget_p->getR_2 ());
...
//continue_:
  ...
  int mode = STREAM_LIB_ALSA_PLAYBACK_DEFAULT_MODE;
...
//     mode |= SND_PCM_ASYNC;
  ...
  {
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("failed to snd_pcm_open(\"%s\",%d) for playback: \"%s\", aborting\n"),
                ACE_TEXT (device_identifier_string.c_str ()),
                mode,
                ACE_TEXT (snd_strerror (result))));
    goto error;
  ...
  (*modulehandler_configuration_iterator).second.second->ALSAConfiguration->handle =
    ui_cb_data_p->handle;
  ACE_DEBUG ((LM_DEBUG,
              ACE_TEXT ("opened ALSA device (playback) \"%s\": %@\n"),
              ACE_TEXT (device_identifier_string.c_str ()),
              ui_cb_data_p->handle));
#endif // ACE_WIN32 || ACE_WIN64
                             ...
                             gpointer userData_in)
{
  ...
  // g_printerr("%s\n", gdk_keyval_name (keyEvent_in->keyval));
...
togglebutton_playback_toggled_cb (GtkToggleButton* toggleButton_in,
                                  gpointer userData_in)
{
  STREAM_TRACE (ACE_TEXT ("::togglebutton_playback_toggled_cb"));
