...
//    event_source_id =
  ...
  std::string format_string =
    Common_String_Tools::toupper (Common_File_Tools::fileExtension (filename_string, false));
  g_value_set_string (&value,
                      format_string.c_str ());
  Common_UI_GTK_Tools::selectValue (combo_box_p,
                                    value,
  ...
  switch (ui_cb_data_base_p->mediaFramework)
  {
    case STREAM_MEDIAFRAMEWORK_DIRECTSHOW:
    {
      (*directshow_stream_iterator).second.second->targetFileName =
        filename_string;
      (*directshow_stream_iterator_2).second.second->targetFileName =
        filename_string;
      break;
    }
    case STREAM_MEDIAFRAMEWORK_MEDIAFOUNDATION:
                                    ...
                                    gpointer userData_in)
{
  ...
  module_p = stream_p->find (module_name_string);
...
void
combobox_save_format_changed_cb (GtkWidget* widget_in,
                                 gpointer userData_in)
{
  STREAM_TRACE (ACE_TEXT ("::combobox_save_format_changed_cb"));
  ...
  switch (ui_cb_data_base_p->mediaFramework)
  {
    ...
    case STREAM_MEDIAFRAMEWORK_MEDIAFOUNDATION:
    {
      ...
      ACE_ASSERT (mediafoundation_cb_data_p->configuration);
      mediafoundation_stream_iterator =
        mediafoundation_cb_data_p->configuration->videoStreamConfiguration.find (ACE_TEXT_ALWAYS_CHAR (STREAM_DEC_DECODER_LIBAV_ENCODER_DEFAULT_NAME_STRING));
      ACE_ASSERT (mediafoundation_stream_iterator != mediafoundation_cb_data_p->configuration->videoStreamConfiguration.end ());
      break;
    }
    default:
  ...
  switch (ui_cb_data_base_p->mediaFramework)
  {
    ...
    case STREAM_MEDIAFRAMEWORK_MEDIAFOUNDATION:
    {
      (*mediafoundation_stream_iterator).second.second->codecConfiguration->codecId =
        static_cast<enum AVCodecID> (g_value_get_int (&value));
      break;
    }
    default:
...
#else
  display_adapter_s =
    Common_UI_Tools::getAdapter ((*iterator_2).second.second->display);
#endif // ACE_WIN32 || ACE_WIN64
  g_value_init (&value, G_TYPE_STRING);
  g_value_set_string (&value,
...
//      allocation_in->width;
  ...
  {
    case STREAM_MEDIAFRAMEWORK_DIRECTSHOW:
      directshow_cb_data_p->videoStream->notify (STREAM_SESSION_MESSAGE_RESIZE,
                                                 false, // recurse upstream ?
                                                 true); // expedite ?
      break;
    case STREAM_MEDIAFRAMEWORK_MEDIAFOUNDATION:
      mediafoundation_cb_data_p->videoStream->notify (STREAM_SESSION_MESSAGE_RESIZE,
                                                      false, // recurse upstream ?
                                                      true); // expedite ?
      break;
    default:
...
#else
  ui_cb_data_p->videoStream->notify (STREAM_SESSION_MESSAGE_RESIZE,
                                     false, // recurse upstream ?
                                     true); // expedite ?
#endif // ACE_WIN32 || ACE_WIN64
                                    ...
                                    gpointer userData_in)
{
  ...
  switch (ui_cb_data_base_p->mediaFramework)
  {
    case STREAM_MEDIAFRAMEWORK_DIRECTSHOW:
    {
      ...
      stream_p = directshow_cb_data_p->videoStream;
      module_name =
        Stream_Visualization_Tools::rendererToModuleName (directshow_cb_data_p->configuration->videoStreamConfiguration.configuration_->renderer);
      break;
    }
    case STREAM_MEDIAFRAMEWORK_MEDIAFOUNDATION:
    {
