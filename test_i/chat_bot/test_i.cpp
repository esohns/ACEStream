...
do_printUsage (const std::string& programName_in)
{
  ...
  std::cout << ACE_TEXT_ALWAYS_CHAR ("-y          : use framework renderer [") // ? (directshow|mediafoundation) renderer : WASAPI|waveOut
            ...
            << std::endl;
#endif // ACE_WIN32 || ACE_WIN64
  std::cout << ACE_TEXT_ALWAYS_CHAR ("-z [STRING] : language [")
                     ...
                     std::string& language_out)
{
  ...
  while ((option = argument_parser ()) != EOF)
  {
    switch (option)
    {
      ...
      case 'z':
      {
        language_out = ACE_TEXT_ALWAYS_CHAR (argument_parser.opt_arg ());
        break;
      }
      // error handling
      case ':':
...
continue_2:
  ...
  if (!useMediaFoundationSource_in)
  {
    Test_I_MediaFoundation_Target* writer_p =
      &const_cast<Test_I_MediaFoundation_Target&> (stream_in.getR_5 ());
...
//  ALSA_configuration_2.asynch = false;
  ...
  switch (mediaFramework_in)
  {
    ...
    case STREAM_MEDIAFRAMEWORK_MEDIAFOUNDATION:
    {
      mediafoundation_modulehandler_configuration.language = language_in;
      mediafoundation_modulehandler_configuration.maximumQueueSlots = 0; // unlimited
