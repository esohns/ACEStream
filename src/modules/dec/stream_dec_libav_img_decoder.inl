...
Stream_Decoder_LibAV_ImageDecoder_T<ACE_SYNCH_USE,
                              ...
                              SessionMessageType,
                              SessionDataContainerType,
                              MediaType>::~Stream_Decoder_LibAV_ImageDecoder_T ()
{
  STREAM_TRACE (ACE_TEXT ("Stream_Decoder_LibAV_ImageDecoder_T::~Stream_Decoder_LibAV_ImageDecoder_T"));
...
//  avcodec_register_all ();
  ...
  if (unlikely (outputFormat_.format == AV_PIX_FMT_NONE))
    ...
    frame_->format = outputFormat_.format;
...
//  } // end FOR
  ...
  message_block_p = inherited::allocateMessage (0);
  ...
  DataMessageType* message_p = dynamic_cast<DataMessageType*> (message_block_p);
  ACE_ASSERT (message_p);

  if (unlikely (!Common_Image_Tools::convert (transformContext_,
