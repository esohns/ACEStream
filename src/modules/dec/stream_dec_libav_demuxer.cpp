...
const char libacestream_default_dec_libav_demuxer_module_name_string[] =
  ACE_TEXT_ALWAYS_CHAR (STREAM_DEC_DECODER_LIBAV_DEMUXER_DEFAULT_NAME_STRING);

//////////////////////////////////////////

int
acestream_libav_demuxer_read_cb (void* userData_in,
                                 uint8_t* buffer_in,
                                 int bufferSize_in)
{
  // sanity check(s)
  struct ACEStream_LibAV_Demuxer_CBData* cb_data_p =
    static_cast<struct ACEStream_LibAV_Demuxer_CBData*> (userData_in);
  ACE_ASSERT (cb_data_p);
  ACE_ASSERT (buffer_in);
  ACE_ASSERT (bufferSize_in);

  bool return_eof_b = false;
  int bytes_read = 0;
...
continue_2:
  if (return_eof_b)
    ...
    goto next; // always return some data
