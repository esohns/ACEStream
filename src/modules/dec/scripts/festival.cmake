...
if(UNIX)
  ...
  if(PKG_FESTIVAL_FOUND)
    ...
    find_library(
      SPEECHTOOLS_BASE_LIBRARY ${SPEECHTOOLS_BASE_LIB_FILE}
      PATHS /usr
      PATH_SUFFIXES lib64
    ...
    find_library(
      SPEECHTOOLS_LIBRARY ${SPEECHTOOLS_LIB_FILE}
      PATHS /usr
      PATH_SUFFIXES lib64
    ...
    find_library(
      SPEECHTOOLS_STRING_LIBRARY ${SPEECHTOOLS_STRING_LIB_FILE}
      PATHS /usr
      PATH_SUFFIXES lib64
      DOC "searching for ${SPEECHTOOLS_STRING_LIB_FILE}")
    if(NOT SPEECHTOOLS_STRING_LIBRARY)
      message(
        WARNING "could not find ${SPEECHTOOLS_STRING_LIB_FILE}, continuing")
    endif(NOT SPEECHTOOLS_STRING_LIBRARY)
    set(FESTIVAL_LIB_FILE "libFestival.a")
    find_library(
      FESTIVAL_LIBRARY ${FESTIVAL_LIB_FILE}
      PATHS /usr
      PATH_SUFFIXES lib64
    ...
    if(SPEECHTOOLS_BASE_LIBRARY
       AND SPEECHTOOLS_LIBRARY
       AND SPEECHTOOLS_STRING_LIBRARY
       AND FESTIVAL_LIBRARY)
      # message (STATUS "Found ${SPEECHTOOLS_BASE_LIB_FILE} library
      # \"${SPEECHTOOLS_BASE_LIBRARY}\"") message (STATUS "Found
      # ${SPEECHTOOLS_LIB_FILE} library \"${SPEECHTOOLS_LIBRARY}\"") message
      # (STATUS "Found ${SPEECHTOOLS_STRING_LIB_FILE} library
      # \"${SPEECHTOOLS_STRING_LIBRARY}\"") message (STATUS "Found
      # ${FESTIVAL_LIB_FILE} library \"${FESTIVAL_LIBRARY}\"")
      set(FESTIVAL_FOUND TRUE)
      set(FESTIVAL_INCLUDE_DIRS "/usr/include/speech_tools")
      set(FESTIVAL_LIBRARIES
          "${FESTIVAL_LIBRARY};${SPEECHTOOLS_BASE_LIBRARY};${SPEECHTOOLS_LIBRARY};${SPEECHTOOLS_STRING_LIBRARY}"
      )
    endif(
      SPEECHTOOLS_BASE_LIBRARY
      AND SPEECHTOOLS_LIBRARY
      AND SPEECHTOOLS_STRING_LIBRARY
      AND FESTIVAL_LIBRARY)
...
elseif(WIN32)
  ...
  if(NOT festival_FOUND)
    ...
    if(SPEECHTOOLS_BASE_LIBRARY
       ...
       AND FESTIVAL_LIBRARY)
      message(
        STATUS "Found ${FESTIVAL_LIB_FILE} library \"${FESTIVAL_LIBRARY}\"")
      set(FESTIVAL_FOUND TRUE)
      set(FESTIVAL_INCLUDE_DIRS
          "$ENV{LIB_ROOT}/speech_tools/include;$ENV{LIB_ROOT}/festival/src/include"
      )
      set(FESTIVAL_LIBRARIES
          "${SPEECHTOOLS_BASE_LIBRARY};${SPEECHTOOLS_LIBRARY};${SPEECHTOOLS_STRING_LIBRARY};${FESTIVAL_LIBRARY}"
      )
      # set (FESTIVAL_LIB_DIR
      # "$ENV{LIB_ROOT}/festival/src/lib/${CMAKE_BUILD_TYPE}")
    endif(
      SPEECHTOOLS_BASE_LIBRARY
      AND SPEECHTOOLS_LIBRARY
      AND SPEECHTOOLS_STRING_LIBRARY
...
# set (FLITE_LIB_DIR "$ENV{LIB_ROOT}/flite/build/x86_64-linux-gnu/lib")
 ...
 if (VCPKG_USE)
  ...
  find_package (flite CONFIG)
    if(FLITE_FOUND)
      ...
      endif(${CMAKE_BUILD_TYPE} STREQUAL "Debug" OR ${CMAKE_BUILD_TYPE}
                                                    STREQUAL "RelWithDebInfo")
    endif(FLITE_FOUND)
  endif(VCPKG_USE)
  if(NOT FLITE_FOUND)
    # set (FLITE_LIB_FILE "flite.lib") find_library (FLITE_LIBRARY
    # ${FLITE_LIB_FILE} PATHS $ENV{LIB_ROOT}/flite/build/x86_64-mingw32
    # PATH_SUFFIXES lib DOC "searching for ${FLITE_LIB_FILE}") if (NOT
    # FLITE_LIBRARY) message (WARNING "could not find ${FLITE_LIB_FILE},
    # continuing") endif (NOT FLITE_LIBRARY)
    set(FLITE_LIB_FILE "libflite.lib")
    find_library(
      FLITE_LIBRARY ${FLITE_LIB_FILE}
    ...
    find_library(
      CMU_LEX_LIBRARY ${CMU_LEX_LIB_FILE}
      PATHS $ENV{LIB_ROOT}/flite/${CMAKE_VS_PLATFORM_TOOLSET_HOST_ARCHITECTURE}
      PATH_SUFFIXES ${CMAKE_BUILD_TYPE}
      DOC "searching for ${CMU_LEX_LIB_FILE}")
    if(NOT CMU_LEX_LIBRARY)
    ...
    find_library(
      USENGLISH_LIBRARY ${USENGLISH_LIB_FILE}
      PATHS $ENV{LIB_ROOT}/flite/${CMAKE_VS_PLATFORM_TOOLSET_HOST_ARCHITECTURE}
      PATH_SUFFIXES ${CMAKE_BUILD_TYPE}
      DOC "searching for ${USENGLISH_LIB_FILE}")
    if(NOT USENGLISH_LIBRARY)
    ...
    find_library(
      CMU_GRAPHEME_LANG_LIBRARY ${CMU_GRAPHEME_LANG_LIB_FILE}
      PATHS $ENV{LIB_ROOT}/flite/${CMAKE_VS_PLATFORM_TOOLSET_HOST_ARCHITECTURE}
      PATH_SUFFIXES ${CMAKE_BUILD_TYPE}
      DOC "searching for ${CMU_GRAPHEME_LANG_LIB_FILE}")
    if(NOT CMU_GRAPHEME_LANG_LIBRARY)
      message(
        WARNING "could not find ${CMU_GRAPHEME_LANG_LIB_FILE}, continuing")
    endif(NOT CMU_GRAPHEME_LANG_LIBRARY)
    set(CMU_GRAPHEME_LEX_LIB_FILE "libflite_cmu_grapheme_lex.lib")
    find_library(
      CMU_GRAPHEME_LEX_LIBRARY ${CMU_GRAPHEME_LEX_LIB_FILE}
      PATHS $ENV{LIB_ROOT}/flite/${CMAKE_VS_PLATFORM_TOOLSET_HOST_ARCHITECTURE}
      PATH_SUFFIXES ${CMAKE_BUILD_TYPE}
      DOC "searching for ${CMU_GRAPHEME_LEX_LIB_FILE}")
    if(NOT CMU_GRAPHEME_LEX_LIBRARY)
    ...
    if(FLITE_LIBRARY
       AND CMU_LEX_LIBRARY
       AND USENGLISH_LIBRARY
       AND CMU_GRAPHEME_LANG_LIBRARY
       AND CMU_GRAPHEME_LEX_LIBRARY)
      # if (FLITE_LIBRARY)
      set(FLITE_FOUND TRUE)
      set(FLITE_INCLUDE_DIRS
          "$ENV{LIB_ROOT}/flite/include;$ENV{LIB_ROOT}/flite/lang")
      set(FLITE_LIBRARIES
          "${FLITE_LIBRARY};${CMU_LEX_LIBRARY};${USENGLISH_LIBRARY};${CMU_GRAPHEME_LANG_LIBRARY};${CMU_GRAPHEME_LEX_LIBRARY}"
      )
      # set (FLITE_LIBRARIES "${FLITE_LIBRARY}")
      set(FLITE_LIB_DIR "$ENV{LIB_ROOT}/flite/build/x86_64-mingw32/lib")
    endif(
      FLITE_LIBRARY
      AND CMU_LEX_LIBRARY
      AND USENGLISH_LIBRARY
