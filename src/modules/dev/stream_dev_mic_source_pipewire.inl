 ...
 , PODBuffer_ ()
{
  ...
  // ACE_OS::memset (PODBuffer_, 0, sizeof (uint8_t[STREAM_DEV_MIC_PIPEWIRE_DEFAULT_POD_BUFFER_SIZE]));
}
                                                                    ...
                                                                    Stream_IAllocator* allocator_in)
{
  ...
  CBData_.displayToConsole = true; // *TODO*: make this configurable
  ...
  switch (message_inout->type ())
  {
    ...
    { // sanity check(s)
      ...
      struct spa_pod_builder POD_builder_s =
        SPA_POD_BUILDER_INIT (PODBuffer_, sizeof (uint8_t[STREAM_DEV_MIC_PIPEWIRE_DEFAULT_POD_BUFFER_SIZE]));
