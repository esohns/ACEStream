                                     ...
                                     Stream_ImageScreen_Stream>::OnInit_2 (IAPPLICATION_T* iapplication_in)
{
  ...
  typename Stream_ImageScreen_WxWidgetsIApplication_t::CALLBACKDATA_T& cb_data_r =
    const_cast<typename Stream_ImageScreen_WxWidgetsIApplication_t::CALLBACKDATA_T&> (application_->getR_2 ());
  ACE_ASSERT (cb_data_r.configuration);
  ...
  Stream_ImageScreen_Stream::CONFIGURATION_T::ITERATOR_T stream_iterator_2 =
    cb_data_r.configuration->streamConfiguration.find (Stream_Visualization_Tools::rendererToModuleName (cb_data_r.configuration->streamConfiguration.configuration_->renderer));
  ...
  togglebutton_save->Enable (!(*stream_iterator).second.second->fileIdentifier.identifier.empty ());
  togglebutton_save->SetValue (!(*stream_iterator).second.second->fileIdentifier.identifier.empty ());
  textctrl_filename->Enable (togglebutton_save->GetValue ());
  textctrl_filename->SetValue (ACE_TEXT_ALWAYS_CHAR (ACE::basename ((*stream_iterator).second.second->fileIdentifier.identifier.c_str (),
                                                                    ACE_DIRECTORY_SEPARATOR_CHAR)));
  //directorypicker_save->Enable (togglebutton_save->GetValue ());
  //directorypicker_save->SetPath (ACE_TEXT_ALWAYS_CHAR (ACE::dirname ((*stream_iterator).second.second->fileIdentifier.identifier.c_str (),
  //                                                                   ACE_DIRECTORY_SEPARATOR_CHAR)));
  ...
  { ACE_ASSERT (!(*stream_iterator_2).second.second->display.device.empty ());
    index_i =
      (initializing_ ? choice_source->FindString (Stream_Device_DirectShow_Tools::devicePathToString ((*stream_iterator_2).second.second->display.device),
                                                  false)
                     : 0);
  ...
  if (likely (activate_display_b))
  {
    index_i =
      (initializing_ ? Common_UI_WxWidgets_Tools::clientDataToIndex (choice_screen,
                                                                     ACE_TEXT_ALWAYS_CHAR ((*stream_iterator_2).second.second->display.device.c_str ()))
                     : 0);
    ACE_ASSERT (index_i != wxNOT_FOUND);
    ...
    event_2.SetInt ((!(*stream_iterator_2).second.second->display.device.empty () ? 1
                                                                                  : 0));
                                     ...
                                     Stream_ImageScreen_Stream>::dialog_main_idle_cb (wxIdleEvent& event_in)
{
  ...
  Stream_ImageScreen_WxWidgetsIApplication_t::CALLBACKDATA_T& cb_data_r =
                                     ...
                                     Stream_ImageScreen_Stream>::togglebutton_record_toggled_cb (wxCommandEvent& event_in)
{
  ...
  ACE_OS::memset (&cb_data_r.progressData.statistic,
                  0,
                  sizeof (struct Stream_Statistic));
...
continue_:
  ...
  bool result =
    Test_U_Tools::spawn<struct Stream_ImageScreen_UI_ThreadData,
                        Stream_ImageScreen_WxWidgetsIApplication_t::CALLBACKDATA_T> (ACE_TEXT_ALWAYS_CHAR (TEST_U_STREAM_THREAD_NAME),
                                     ...
                                     Stream_ImageScreen_Stream>::choice_framerate_changed_cb (wxCommandEvent& event_in)
{
  ...
  //spinctrl_framesize->SetValue (Stream_MediaFramework_DirectShow_Tools::toFramesize (cb_data_r.configuration->streamConfiguration.configuration_->format));
  //IBaseFilter* filter_p = NULL;
  //HRESULT result =
  //  (*stream_iterator).second.second->builder->FindFilterByName (STREAM_DEV_CAM_DIRECTSHOW_FILTER_NAME_CAPTURE_VIDEO,
  //                                                              &filter_p);
                                     ...
                                     Stream_ImageScreen_Stream>::choice_screen_changed_cb (wxCommandEvent& event_in)
{
  ...
  ACE_ASSERT (stream_iterator != cb_data_r.configuration->streamConfiguration.end ());
  wxStringClientData* client_data_p =
    dynamic_cast<wxStringClientData*> (choice_displayadapter->GetClientObject (event_in.GetSelection ()));
  ACE_ASSERT (client_data_p);
  (*stream_iterator).second.second->display.device =
    client_data_p->GetData ().ToStdString ();
  ACE_ASSERT (!(*stream_iterator).second.second->display.device.empty ());
  ...
  Common_UI_Resolutions_t resolutions_a =
    Common_UI_Tools::get ((*stream_iterator).second.second->display.device);
  ACE_ASSERT (!resolutions_a.empty ());
  Common_UI_Resolution_t resolution_s =
