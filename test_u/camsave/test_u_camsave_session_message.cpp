...
Stream_CamSave_SessionMessage::Stream_CamSave_SessionMessage (const Stream_CamSave_SessionMessage& message_in)
...
ACE_Message_Block*
Stream_CamSave_SessionMessage::duplicate (void) const
{
  STREAM_TRACE (ACE_TEXT ("Stream_CamSave_SessionMessage::duplicate"));

  Stream_CamSave_SessionMessage* message_p = NULL;

  // create a new <Stream_CamSave_SessionMessage> that contains unique copies of
  ...
  if (inherited::message_block_allocator_ == NULL)
  {
    ACE_NEW_RETURN (message_p,
                    Stream_CamSave_SessionMessage (*this),
                    NULL);
  } // end IF

  // *WARNING*: the allocator returns a Stream_CamSave_SessionMessageBase<ConfigurationType>
  // when passing 0 as argument to malloc()...
  ACE_NEW_MALLOC_RETURN (message_p,
                         static_cast<Stream_CamSave_SessionMessage*> (inherited::message_block_allocator_->malloc (0)),
                         Stream_CamSave_SessionMessage (*this),
                         NULL);
