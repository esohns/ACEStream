...
main ()
{
  vec2 uv = (vec2(iResolution.x-gl_FragCoord.x, gl_FragCoord.y) / iResolution.xy) * (1. - 2.*zoom) + vec2(zoom, zoom);
  float niceNoise1 = FBM( vec3(80.0 * uv, speed * 60.*iTime));
  float niceNoise2 = FBM( vec3(80.0 * uv, speed * 62.*iTime + 300.));
  gl_FragColor = texture(iChannel0, uv + vec2(magn*0.2*niceNoise1,magn*0.21*niceNoise2) );
}
