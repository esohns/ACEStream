...
const char libacestream_default_module_opengl_glut_name_string[] =
  ACE_TEXT_ALWAYS_CHAR ("OpenGL_GLUT");

#if defined (ACE_WIN32) || defined (ACE_WIN64)
Test_U_CameraFilter_OpenGL_GLUT::Test_U_CameraFilter_OpenGL_GLUT (ISTREAM_T* stream_in)
#else
Test_U_CameraFilter_OpenGL_GLUT::Test_U_CameraFilter_OpenGL_GLUT (typename inherited::ISTREAM_T* stream_in)
 ...
 , window_ (0)
{
  STREAM_TRACE (ACE_TEXT ("Test_U_CameraFilter_OpenGL_GLUT::Test_U_CameraFilter_OpenGL_GLUT"));
                                                       ...
                                                       bool& passMessageDownstream_out)
{
  ...
  switch (message_inout->type ())
  {
    case STREAM_SESSION_MESSAGE_BEGIN:
    {
      ...
      if (!Common_GL_Tools::loadAndCompileShaderFile (ACE_TEXT_ALWAYS_CHAR (TEST_U_VERTEX_SHADER_FILENAME),
                                                      GL_VERTEX_SHADER,
                                                      vertex_shader_id))
      {
        ACE_DEBUG ((LM_ERROR,
      ...
      if (!Common_GL_Tools::loadAndCompileShaderFile (ACE_TEXT_ALWAYS_CHAR (TEST_U_FRAGMENT_SHADER_FILENAME),
                                                      GL_FRAGMENT_SHADER,
                                                      fragment_shader_id))
      {
        ACE_DEBUG ((LM_ERROR,
      ...
      CBData_.textureSizeLoc =
        glGetUniformLocation (CBData_.programId, ACE_TEXT_ALWAYS_CHAR ("texSize"));
      //ACE_ASSERT (CBData_.textureSizeLoc != -1);
      ...
      glutCloseFunc (camera_filter_glut_close);
      glutDisplayFunc (camera_filter_glut_draw);
      glutReshapeFunc (camera_filter_glut_reshape);
      glutVisibilityFunc (camera_filter_glut_visible);
      glutKeyboardFunc (camera_filter_glut_key);
...
int
Test_U_CameraFilter_OpenGL_GLUT::svc (void)
{
  STREAM_TRACE (ACE_TEXT ("Test_U_CameraFilter_OpenGL_GLUT::svc"));
...
void
camera_filter_glut_reshape (int width_in, int height_in)
{
  glViewport (0, 0,
...
void
camera_filter_glut_key (unsigned char key_in, int x_in, int y_in)
{
  struct Test_U_CameraFilter_OpenGL_GLUT_WindowData* cb_data_p =
    static_cast<struct Test_U_CameraFilter_OpenGL_GLUT_WindowData*> (glutGetWindowData ());
...
void
camera_filter_glut_idle (void)
{
  glutPostRedisplay ();
}

void
camera_filter_glut_close ()
{ // pressing the 'x' is the same as hitting 'Escape'
  camera_filter_glut_key (27, 0, 0);
}

void
camera_filter_glut_visible (int vis)
{
  if (vis == GLUT_VISIBLE)
    glutIdleFunc (camera_filter_glut_idle);
  else
    glutIdleFunc (NULL);
}
