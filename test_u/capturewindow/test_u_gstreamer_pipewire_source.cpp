...
const char test_u_gstreamer_pipewire_source_module_name_string[] =
  ACE_TEXT_ALWAYS_CHAR (STREAM_DEV_CAM_SOURCE_GSTREAMER_DEFAULT_NAME_STRING);

//////////////////////////////////////////

gboolean
test_u_gstreamer_pipewire_source_bus_cb (GstBus* bus_in,
                                         GstMessage* message_in,
                                         gpointer data_in)
{
  STREAM_TRACE (ACE_TEXT ("test_u_gstreamer_pipewire_source_bus_cb"));

  // sanity check(s)
  struct Test_U_GStreamer_Pipewire_Source_CBData* cb_data_p =
    static_cast<struct Test_U_GStreamer_Pipewire_Source_CBData*> (data_in);
...
GstFlowReturn
test_u_gstreamer_pipewire_source_new_sample_cb (GstElement* sink_in,
                                                gpointer userData_in)
{
  // sanity check(s)
  struct Test_U_GStreamer_Pipewire_Source_CBData* cb_data_p =
    static_cast<struct Test_U_GStreamer_Pipewire_Source_CBData*> (userData_in);
  ACE_ASSERT (cb_data_p);
