...
Branch_Module_EventHandler::Branch_Module_EventHandler (ISTREAM_T* stream_in)
 : inherited (stream_in)
{
  STREAM_TRACE (ACE_TEXT ("Branch_Module_EventHandler::Branch_Module_EventHandler"));
...
Branch_Module_EventHandler:: clone () const
{
  ...
  if (!module_p)
    ACE_DEBUG ((LM_CRITICAL,
                ...
                inherited::mod_->name (),
                sizeof (Branch_Module_EventHandler_Module)));
  else
  {
    task_p = module_p->writer ();
    ACE_ASSERT (task_p);

    Branch_Module_EventHandler* eventHandler_impl =
        dynamic_cast<Branch_Module_EventHandler*> (task_p);
    if (!eventHandler_impl)
    {
      ACE_DEBUG ((LM_ERROR,
                  ACE_TEXT ("%s: dynamic_cast<Branch_Module_EventHandler> failed, aborting\n"),
                  inherited::mod_->name ()));
